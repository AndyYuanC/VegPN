    ...
    def __init__(self, root=dataset_path, npoints=2500, classification=False, class_choice=None, split='train', normalize=True):
        ...
        for item in self.cat:
            ...
            for fn in fns:
                ...
                self.meta[item].append((os.path.join(dir_point, token + '.pts'),self.cat[item], token))            
        self.datapath = []
        for item in self.cat:
...
#            point_set, seg, cls= self.cache[index]
            ...
            point_set,cls, foldername, filename = self.cache[index]
        else:
...
#            cls = np.array([cls]).astype(np.int32)
            ...
            if len(self.cache) < self.cache_size:
                self.cache[index] = (point_set, cls, foldername, filename)
